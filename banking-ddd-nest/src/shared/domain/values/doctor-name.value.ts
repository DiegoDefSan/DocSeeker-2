...
export class DoctorName {
  ...
  public static create(firstName: string, lastName: string): DoctorName {
    firstName = (firstName ?? "").trim();
    lastName = (lastName ?? "").trim();
    return new DoctorName(firstName, lastName);
  }

  public static createv2(firstName: string, lastName: string): Result<AppNotification, DoctorName> {
    ...
    return Result.ok(new DoctorName(firstName, lastName));
  }
}